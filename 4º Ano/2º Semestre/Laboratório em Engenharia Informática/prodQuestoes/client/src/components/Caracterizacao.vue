  ...
  <v-form v-model="valid" ref="form" >
    <v-container>
      <v-row dense> 
        ...
        <v-col cols="12" md="4">
          <v-autocomplete v-model="formData.domain"
        ...
        <v-col cols="12" md="4">
          <v-autocomplete v-model="formData.subdomain" :items="idSubs" :rules="[...rules.required,...rules.length100]" label="Subdomínio"/>
        </v-col>
      </v-row>

      <v-row dense>
        <v-col cols="12" md="4" >
          <v-text-field v-model="formData.study_cycle" :rules="[...rules.required,...rules.length100]" readonly label="Ciclo de Estudos"/>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field v-model="formData.scholarity" :rules="[...rules.required,...rules.length75]" readonly label="Escolaridade"/>
        </v-col>
      </v-row>
      ...
      <v-row >
        ...
        <v-radio-group v-model="formData.display_mode" row mandatory>
          <v-col cols="4" md="4" sm="4" xm="1">
            <v-radio label="G" value="G"/>
          </v-col>
          <v-col cols="2" md="4" sm="4" xm="1">
      ...
      <v-row dense>
        <v-col cols="8" >
          <v-autocomplete v-model="formData.precedencias" :items="idQuestoes" dense chips label="Precedências" multiple/>
        </v-col>
      </v-row>
    </v-container>
  </v-form>
...
export default {
  ...
  methods: {
    ...
    onChange(){
      this.doms.forEach((obj) =>{
        if(this.formData.domain == obj.id){
          this.idSubs.push(obj.body[0].subdomain)
          this.formData.study_cycle = obj.study_cycle
          this.formData.scholarity = obj.scholarity
