<template>
  <v-form v-model="valid" ref="form">
    <v-container>
      ...
      <v-row dense>
        <v-col cols="6">
          <v-text-field v-model="formData.study_cycle" :rules="[...rules.required,...rules.length75]" :counter="75" label="Ciclo de Estudos"/>
        </v-col>
        <v-col cols="6">
      ...
      <v-row no-gutters>
        <v-col sm="1" md="1" lg="1" xl="1" class="mt-4">
...
export default {
  ...
  watch: {
      formData: {
          handler: function() {
            this.$emit('newdataDominio', [this.formData.id,this.formData.description,this.formData.study_cycle,this.formData.scholarity,
