            ...
            if routable:
                ...
                if ip_addr_inter == destination_ip :
                    ...
                    if destination_ip not in self.arp_table:
                        ...
                        arp1 = arp()
                        arp1.opcode = arp.REQUEST # it is an ARP request after all
                        arp1.protosrc = IPAddr(ip_addr_inter)  # to the ip address of the router outgoing interface, you can use the routing table to obtain that 
                        arp1.protodst = IPAddr(destination_ip)    # var of your arp object to the destination ip address
                        # NOTE: you can use the method IPAddr(destination_ip) to pasrse a string to an IP addr.

                        mac_addr_inter = EthAddr(self.arp_table[ip_addr_inter])
                        arp1.hwsrc = mac_addr_inter  # var of your arp object to the mac address of the router outgoing interface, you can use the arp table to obtain that 
                        arp1.hwdst = EthAddr('00:00:00:00:00:00') # var of your arp object to an all zeros mac address '00:00:00:00:00:00'
