...
\textbf{Nota}: De forma a facilitar a visualização do gráfico, assume-se |g = g_eval_exp|.
...
\end{eqnarray*}
\begin{code}
g_eval_exp a = either (g1 a) g2
 where
  g1 a () = a
  g2 = either g3 g4
  g3 b = b
  g4 = either g5 g6
  g5 (op,(b,c)) = if (op == Sum) then b + c else b * c
  g6 (op, b) = if (op == Negate) then -b else expd b
\end{code}
...
Aproveitando o facto de |X * 0 = 0|, para todo e qualquer X, e |e^0 = 1| podemos adicionar estas exceções
...
\\
|ExpAr a|
    \ar[d]_-{|cataExpAr (gopt a)|}
    \ar@@/^1pc/[r]^-{|outExpAr|}
&
|1 + a + Op >< (ExpAr a)|^2| + Op >< ExpAr a|
    \ar@@/^1pc/[l]^-{|inExpAr|}
    \ar[d]^{|recExpAr (cataExpAr gopt a)|}
\\
|a|
&
|1 + a + Op >< a|^2| + Op >< a|
...
clean (Bin (Product) (a) (b)) = if (a == N 0 || b == N 0) then i2 (i1 0) else i2 (i2 (i1 (Product, (a,b))))
clean (Un (Negate) a) = i2 (i2 (i2 (Negate, a)))
clean (Un (E) a) = if (a == N 0) then i2 (i1 1) else i2 (i2 (i2 (E, a)))
---
gopt a = either (g1 a) g2
 where
  g1 a () = a
  g2 = either g3 g4
  g3 b = b
...
|(ExpAr a)|^2
&
|1 + a + Op >< (ExpAr a)|^2| >< (ExpAr a)|^2| + Op >< (ExpAr a)|^2
    \ar[l]^-{|sd_gen|}
}
\end{eqnarray*}
\begin{code}
sd_gen :: Floating a => Either ()
                               (Either a
...
sd_gen = either g1 g2
 where
  ...
  g3 a = (N a, N 0)
  g4 = either g5 g6
  g5 (Sum,((a1,a2),(b1,b2))) = (Bin Sum a1 b1, Bin Sum a2 b2)
  g5 (Product,((a1,a2),(b1,b2))) = (Bin Product a1 b1, Bin Sum (Bin Product a1 b2) (Bin Product a2 b1))
  g6 (Negate, (a1,a2)) = (Un Negate a1, Un Negate a2)
  g6 (E, (a1,a2)) = (Un E a1, Bin Product (Un E a1) a2)
\end{code}
...
\begin{code}
loop (c,h,s) = (div (c*2*h) s, h+2, s+1)
inic = (1,1,2)
prj (c,h,s) = c
\end{code}
por forma a que
...
Assumindo: |c n = div (fac (2n)) (fac (n + 1) * fac n)| e fazendo uns pequenos cálculos podemos afirmar que,
\begin{eqnarray*}
\start
%
        |lcbr(
    ...
    c (n+1) = c n * div (2*(2n + 1)) (n+2)
  )|
%
\qed
\end{eqnarray*}
Assim, consideremos: |h n = 2n + 1| e |s n = n + 2|
\begin{eqnarray*}
\start
%
        |lcbr(
...
\qed
\end{eqnarray*}
Podemos então concluir que: |c (n+1) = div (c n * 2 * h n) (s n)|
...
calcLine = cataList h where
   ...
   --  h2 (a,g) = [f1,f2 a g]
   --  f1 = nil
   --  (f2 (a,g) (x:xs)) z = concat $ (sequenceA [singl . linear1d a x, g xs]) z
---
deCasteljau :: [NPoint] -> OverTime NPoint
...
\end{code}
Gráfico correspondente ao catamorfismo |avg|:
\begin{eqnarray*}
\xymatrix@@C=3cm@@R=2cm{
|A|^+
    \ar[d]_-{|avg|}
...
\end{eqnarray*}
Gráfico correspondente ao catamorfismo |length|:
\begin{eqnarray*}
\xymatrix@@C=3cm@@R=2cm{
|A|^+
    \ar[d]_-{|length|}
...
\end{eqnarray*}
\begin{eqnarray*}
|split avg length = cata (split (either a1 a2) (either l1 l2))|
\end{eqnarray*}
...
\end{eqnarray*}
\begin{eqnarray*}
|split avg length = cata (either (split a1 l1) (split a2 l2))|
\end{eqnarray*}
Temos agora condições para calcular |avg_aux = split avg length|:
\begin{code}
avg_aux = cataL (either (split id (fromIntegral . one)) (split divisao (succ . p2 . p2)))
 where
  divisao = uncurry (/) . adicao
  adicao = split (uncurry (+) . multiplicacao) (succ . p2 . p2)
  multiplicacao = split p1 (uncurry (*) . p2)
\end{code}
A definição |pointwise| do que vemos em cima será:
\begin{eqnarray*}
    |a1 a = a|
\end{eqnarray*}
\begin{eqnarray*}
...
Gráfico correspondente ao catamorfismo |avgLTree|:
\begin{eqnarray*}
\xymatrix@@C=3cm@@R=2cm{
|LTree A|
    \ar[d]_-{|avgLTree|}
&
|A + LTree A >< LTree A|
    \ar[d]^{|id + split avgLTree lengthLTree >< split avgLTree lengthLTree|}
    \ar[l]_-{|inLTree|}
\\
|A|
&
...
\end{eqnarray*}
Gráfico correspondente ao catamorfismo |length| para LTrees:
\begin{eqnarray*}
\xymatrix@@C=3cm@@R=2cm{
|LTree A|
    \ar[d]_-{|lengthLTree|}
...
\begin{eqnarray*}
|split avgLTree lengthLTree = cata (split (either a1 a2) (either l1 l2))|
\end{eqnarray*}
\begin{eqnarray*}
|split avgLTree lengthLTree = cata (either (split a1 l1) (either a2 l2))|
\end{eqnarray*}
Podemos agora definir |gene|:
\begin{code}
avgLTree = p1.cataLTree gene
 where
  gene = either (split id (fromIntegral . one)) (split a2 l2)
  a2 = uncurry (/) . (split x l2)
  l2 = uncurry (+) . (split (p2 . p1) (p2 . p2))
  x  = uncurry (+) . (split y z)
  y  = uncurry (*) . p1
  z  = uncurry (*) . p2
\end{code}
...
\end{eqnarray*}
\begin{eqnarray*}
    |a2 ((m1,n1),(m2,n2)) = (m1*n1 + m2*n2) / (n1 + n2)|
\end{eqnarray*}
\begin{eqnarray*}
    |l1 a = 1|
\end{eqnarray*}
\begin{eqnarray*}
    |l2 ((m1,n1),(m2,n2)) = n1 + n2|
\end{eqnarray*}
